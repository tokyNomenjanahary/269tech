...
class PlayController extends Controller
{
    ...
    public function spinWheel()
    {
        $game = Game::find(3);
        //$pageTitle = "Play " . $game->name;
        $pageTitle = "1 in 2 chance of winning ";
        $page = "playspin";
        return view(activeTemplate() . 'user.games.spinWheel', compact('game', 'pageTitle', 'page'));
    }
    public function calculeRebours()
    {
        // die("here");
        $lasts = Tirage::all();
        // die("here");
        //$new_date = date( "H:i:s",strtotime($last->date));
        $heures   = 0;  // les heures < 24 2021-22-22 22:22:22
         ...
         ***************************************************************************/
        $secondes = mktime(
            date("H", strtotime($lasts->date)) + $heures,
            date("i", strtotime($lasts->date)) + $minutes,
            date("s", strtotime($lasts->date)) + $secondes,
            $mois,
            $jour,
            $annee
        ) - time();
